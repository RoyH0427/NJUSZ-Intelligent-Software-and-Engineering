...
def constructBayesNet(gameState: hunters.GameState):
    ...
    allPossiblePositions = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    allPossibleObservedDistances = list(range(0, MAX_NOISE + X_RANGE + Y_RANGE - 1))
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for elimination in eliminationOrder:
            newFactors, newfactor = joinFactorsByVariable(Factors, elimination)
            if len(newfactor.unconditionedVariables()) > 1:
                fullJoint = eliminate(newfactor, elimination)
                Factors = newFactors
                Factors.append(fullJoint)
            else :
                Factors = newFactors
            answer_Factors = joinFactors(Factors)
        return normalize(answer_Factors)
        # raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        num_particles_per_position = num_particles // num_legal_positions
        for position in legal_positions:
            for i in range(num_particles_per_position):
                self.particles.append(position)
        num_particles_left = num_particles - num_particles_per_position * num_legal_positions
        for i in range(num_particles_left):
